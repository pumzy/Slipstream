...
var _platformArrays = __webpack_require__(5);
...
var initialPlatforms = exports.initialPlatforms = function initialPlatforms() {
  return [{ x: 200, y: 230, speed: 3, obstacles: null, powercubes: null, popped: [], width: 800, id: 1 }, { x: 1050, y: 350, speed: 3, obstacles: null, powercubes: null, popped: [], width: 800, id: 2 }, { x: 1800, y: 300, speed: 3, obstacles: null, powercubes: null, popped: [], width: 800, id: 3 }, { x: 2650, y: 250, speed: 3, obstacles: null, powercubes: null, popped: [], width: 800, id: 4 }];
};
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  scoresRef.orderByChild("score").limitToLast(5).on('value', function (snapshot, highscores) {
  ...
  var startScreen = function startScreen() {
    if (started === false) {
      ...
      if (readCookie("slipstream_username") !== null) {
        ...
        var username = window.prompt("Hello Friend! Please Enter a Username");
        if (username) {
  ...
  var inPCA = function inPCA(platform) {
    ...
    for (var j = 0; j < cubes.length; j++) {
      if (player.x < cubes[j].x - 24 && player.x > cubes[j].x - 33 && player.y >= cubes[j].y - 30 && onPlatform(platforms) && player.y <= cubes[j].y + 10) {
        return cubes[j];
      }
    }
    return false;
